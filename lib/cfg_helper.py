...
def cfg_solvef(cmd, root, path):
    ...
    if cmd.find("MODEL") == 0:
        ...
        return model_cfg_bank(path)(goto)
...
def cfg_solve(cfg, cfg_root, path):
    ...
    if isinstance(cfg, list):
        for i in range(len(cfg)):
            if isinstance(cfg[i], (list, dict)):
                cfg[i] = cfg_solve(cfg[i], cfg_root, path)
            else:
                cfg[i] = cfg_solvef(cfg[i], cfg_root, path)
    if isinstance(cfg, dict):
        for k in cfg:
            if isinstance(cfg[k], (list, dict)):
                cfg[k] = cfg_solve(cfg[k], cfg_root, path)
            else:
                cfg[k] = cfg_solvef(cfg[k], cfg_root, path)
    return cfg
...
class model_cfg_bank(object):
    def __init__(self, path):
        self.path = path
        self.cfg_dir = osp.join(path, "configs", "model")
        self.cfg_bank = edict()
    ...
    def __call__(self, name):
        ...
        # HACK - Update paths to use the right root
        if name in self.cfg_bank and "pth" in self.cfg_bank[name]:
            self.cfg_bank[name]["pth"] = osp.join(self.path, self.cfg_bank[name]["pth"])
        for f in [
            "tokenizer_decoder",
            "tokenizer_encoder",
            ...
            for ff in ["merges_file", "vocab_file"]:
                if f in self.cfg_bank and ff in self.cfg_bank[f]["args"]:
                    print(f, ff)
                    self.cfg_bank[f]["args"][ff] = osp.join(
                        self.path, self.cfg_bank[f]["args"][ff]
                    )

        cfg = self.cfg_bank[name]
        cfg.name = name
        if "super_cfg" not in cfg:
            cfg = cfg_solve(cfg, cfg, self.path)
    ...
    def get_yaml_path(self, name):
        if name.find("ldm") == 0:
            return osp.join(self.cfg_dir, "ldm.yaml")
        elif name.find("comodgan") == 0:
            return osp.join(self.cfg_dir, "comodgan.yaml")
        elif name.find("stylegan") == 0:
            return osp.join(self.cfg_dir, "stylegan.yaml")
        elif name.find("absgan") == 0:
            return osp.join(self.cfg_dir, "absgan.yaml")
        elif name.find("ashgan") == 0:
            return osp.join(self.cfg_dir, "ashgan.yaml")
        elif name.find("sr3") == 0:
            return osp.join(self.cfg_dir, "sr3.yaml")
        elif name.find("specdiffsr") == 0:
            return osp.join(self.cfg_dir, "specdiffsr.yaml")
